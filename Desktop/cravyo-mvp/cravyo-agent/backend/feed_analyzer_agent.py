...
class FeedState(TypedDict):
    ...
    multi_platform_cravings: list[dict]
    fusion_result: Optional[dict]
    meal_nudge: Optional[dict]
...
def call_swiggy(state: FeedState) -> FeedState:
    ...
    if fusion.get("recommended_combo"):
        final_alert = final_alert + "\n\n[FUSION] " + fusion["reasoning"]

    nudge = state.get("meal_nudge") or {}
...
def fuse_cravings(state: FeedState) -> FeedState:
    cravings = state.get("multi_platform_cravings") or []
    dish = state.get("dish") or state.get("cuisine") or ""
    cuisine = state.get("cuisine") or ""

    if not cravings and dish:
        cravings = [{"platform": state["platform"], "food": dish,
                     "confidence": state.get("confidence", 0.5),
                     "detected_at": datetime.now().isoformat(),
                     "frequency": 1}]

    fusion = fusion_engine(cravings)
    nudge = get_meal_nudge(fusion["fused_dish"], fusion["fused_cuisine"],
                           urgency_override=fusion["urgency"])

    print(f"[FUSION] dish={fusion['fused_dish']} urgency={fusion['urgency']}")
    print(f"[NUDGE] window={nudge['meal_window']} urgency={nudge['urgency']}")

    return {**state, "fusion_result": dict(fusion), "meal_nudge": dict(nudge)}
...
graph.add_node("fuse_cravings", fuse_cravings)
...
graph.add_edge("compare_prices", "fuse_cravings")
graph.add_edge("fuse_cravings", "call_swiggy")
...
def run_agent(username: str, platform: str = "instagram",
              ...
              location: dict = None,
              multi_platform_cravings: list[dict] | None = None) -> str:
    if location is None:
    ...
    initial_state = FeedState(
        ...
        multi_platform_cravings=multi_platform_cravings or [],
        fusion_result=None, meal_nudge=None,
    )
    ...
    while True:
        ...
        if choice == "1":
            ...
            test_state = FeedState(
                ...
                dish=None, city="Pune",
                price_comparison=None, price_comparison_text=None,
                multi_platform_cravings=[], fusion_result=None, meal_nudge=None,
            )
